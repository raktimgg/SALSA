...
from .spherical_transformer import SphereFormer
...
class UBlock(nn.Module):
    ...
    def forward(self, inp, xyz, batch):
        ...
        output = self.blocks(inp)  # residual blocks
        # transformer
...
class Semantic(nn.Module):
    def __init__(self,
        ...
        # dpr = [drop_path_rate]*7

        #### backbone
        self.input_conv = spconv.SparseSequential(
            # spconv.SubMConv3d(input_c, m, kernel_size=3, padding=1, bias=False, indice_key='subm1',algo=spconv.ConvAlgo.Native).cuda()
